...
class WeightSyncWorker(Worker):
    ...
    def update_named_param(self, name: str, dtype: torch.dtype, shape: Sequence[int]) -> None:
        ...
        if name == "model.layers.25.self_attn.q_proj.weight":
            print(name, weight)
...
def main(script_args: ScriptArguments):
    ...
    class GenerateResponse(BaseModel):
        ...
        texts: list[list[str]]
        logprobs: list[list[list[list[tuple]]]]
    ...
    async def generate(request: GenerateRequest):
        ...
        all_outputs = sorted(all_outputs, key=lambda x: int(x.request_id))
        texts = [[out.text for out in outputs.outputs] for outputs in all_outputs]
        ...
        return {"completion_ids": completion_ids, "texts": texts, "logprobs": logprobs}
