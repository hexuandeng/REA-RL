...
def main():
    ...
    def preprocess_function(examples):
        with CaptureLogger(tok_logger) as cl:
            _text = examples[text_column_name]  # may have multiple strings
            if "prefix" in column_names:
                _prefix = examples["prefix"]  # may have multiple strings
                prefix = [tokenizer.apply_chat_template([{"role": "user", "content": s}], tokenize=False, add_generation_prompt=True) for s in _prefix]
                text = [s + t + tokenizer.eos_token for s, t in zip(prefix, _text)]
